...
/**
 * Actualiza los campos de un proveedor en PROV_MAESTRO.
 */
function PROV_ACTUALIZAR_PROVEEDOR(datos) {
  if (!datos || !datos.ID_PROVEEDOR) throw new Error("ID_PROVEEDOR es obligatorio para actualizar.");
  ...
  if (filaModificar === -1) {
    throw new Error("No se encontró el proveedor '" + idProveedor + "' para actualizar.");
  }
  ...
  try {
    const hojaHist = ss.getSheetByName(PROV_CONFIG.HOJA_HISTORIAL);
    if (hojaHist) {
      hojaHist.appendRow([
        "HIS-PROV-" + String(Math.max(1, hojaHist.getLastRow())).padStart(6, "0"),
        idProveedor, new Date().toLocaleDateString(), new Date().toLocaleTimeString(),
        Session.getActiveUser().getEmail() || "SISTEMA", "MODIFICACION", "ESTADO",
        valoresFilaActual[encabezados.indexOf("ESTADO")], proveedorActualizado.ESTADO, "Cambio de datos"
      ]);
    }
  } catch (err) {
    console.error("Error al registrar historial proveedor.");
  }
 ...
 */
function PROV_INACTIVAR_PROVEEDOR(idProveedor) {
  return PROV_ACTUALIZAR_PROVEEDOR({ ID_PROVEEDOR: idProveedor, ESTADO: "INACTIVO" });
}

/**
 * Busca un proveedor por NIT, Razón Social o ID del ERP.
 */
