...
/**************************************************************
* 23_SEGURIDAD.gs (VERSIÓN 24.0 - V2 ERP - LIBRO 1)
* RESPONSABILIDAD:
...
function SEG_OBTENER_ENCABEZADOS(nombreHoja) {
  ...
  // ✔️ CORREGIDO: Se agrega [0] para mapear las columnas de la primera fila en lugar de la fila completa como array
...
function SEG_ESTABLECER_CONTRASENA(idUsuario, nuevaContrasena, usuarioActualizacion, tokenSesion) {
  ...
  const validacionSecurity = SEG_VALIDAR_SEGURIDAD_CONTRASENA(nuevaContrasena);
  if (!validacionSecurity.VALIDA) throw new Error(validacionSecurity.MENSAJES.join(" "));
...
/**************************************************************
* 23.02 CONTROL DINÁMICO DE ACCESOS Y MENÚ DE USUARIO (DASHBOARD ROUTING)
* RESPONSABILIDAD: Retornar los módulos permitidos para el rol asociado al token de sesión.
**************************************************************/
function SEG_OBTENER_MENU_NIVEL(tokenSesion) {
  try {
    ...
    const rol = SEG_CONSULTAR_ROL(sesion.ID_ROL);
    const idRolUsuario = sesion.ID_ROL || "ROL-000001";
    
    const modulosMaestrosTodos = ["CLIENTES", "VENTAS", "COMPRAS", "PRODUCTOS", "INVENTARIO", "FINANZAS", "PLANEACION", "OBRAS", "NOMINA", "SEGURIDAD"];

    if (idRolUsuario === "ROL-000001" || (rol && String(rol.NOMBRE_ROL).toUpperCase() === "ADMINISTRADOR")) {
      return {
        EXITO: true,
        ES_ADMIN: true,
        ROL: rol ? rol.NOMBRE_ROL : "ADMINISTRADOR",
        MODULOS: modulosMaestrosTodos
      };
    }
    
    const permisos = SEG_OBTENER_PERMISOS_ROL(idRolUsuario);
    const modulosPermitidos = [];
    
    permisos.forEach(p => {
      const moduloUpper = String(p.MODULO || "").trim().toUpperCase();
      const accionUpper = String(p.ACCION || "").trim().toUpperCase();
      const permitidoUpper = String(p.PERMITIDO || "").trim().toUpperCase();
      const estadoUpper = String(p.ESTADO_PERMISO || "ACTIVO").trim().toUpperCase();
      
      if (accionUpper === "VER" && 
          ["SI", "SÍ", "TRUE", "1", "VERDADERO"].includes(permitidoUpper) &&
          estadoUpper === "ACTIVO") {
        if (!modulosPermitidos.includes(moduloUpper)) {
          modulosPermitidos.push(moduloUpper);
        }
      }
    });

    if (modulosPermitidos.length === 0) {
      return { EXITO: true, ES_ADMIN: false, ROL: rol ? rol.NOMBRE_ROL : idRolUsuario, MODULOS: modulosMaestrosTodos };
    }
    
    return {
      EXITO: true,
      ES_ADMIN: false,
      ROL: rol ? rol.NOMBRE_ROL : idRolUsuario,
      MODULOS: modulosPermitidos
    };
  } catch (error) {
    return { EXITO: false, MENSAJE: "Error interno al verificar menú de nivel: " + error.toString() };
  }
}


/**
 * RPC: Guardar/Modificar Permiso por Rol de forma dinámica desde el Frontend SPA
 * Soporta invocación con objeto datos {ID_PERMISO, ID_ROL, MODULO, SUBMODULO, ACCION, PERMITIDO} 
 * o parámetros posicionales (idRol, modulo, permitido, tokenSesion).
 */
function SEG_GUARDAR_PERMISO_WEB(idRolOrObj, moduloOrToken, permitidoVal, tokenSesionParam) {
  try {
    let idRol, modulo, submodulo, accion, permitido, tokenSesion;
    
    if (typeof idRolOrObj === "object" && idRolOrObj !== null) {
      const datos = idRolOrObj;
      idRol = datos.ID_ROL || "ROL-000001";
      modulo = datos.MODULO || "CLIENTES";
      submodulo = datos.SUBMODULO || "GENERAL";
      accion = datos.ACCION || "VER";
      permitido = datos.PERMITIDO || "SI";
      tokenSesion = moduloOrToken;
    } else {
      idRol = idRolOrObj;
      modulo = moduloOrToken;
      submodulo = "GENERAL";
      accion = "VER";
      permitido = permitidoVal;
      tokenSesion = tokenSesionParam;
    }

    const auth = SEG_VERIFICAR_CONTEXTO_Y_ACCESO(tokenSesion, "SEGURIDAD", "EDITAR");
    const ss = SpreadsheetApp.getActiveSpreadsheet();
    let hoja = ss.getSheetByName(SEG_CONFIG.HOJA_PERMISOS);
    if (!hoja) throw new Error("Hoja USR_PERMISOS no encontrada en la base de datos.");
    ...
    if (ultimaFila >= 2) {
      ...
      for (let i = 0; i < pData.length; i++) {
        ...
        if (rowRol === String(idRol).trim() && 
            rowMod === String(modulo).trim().toUpperCase() && 
            rowAcc === String(accion).trim().toUpperCase()) {
          filaIndex = i + 2;
          break;
        }
      }
    ...
    const estadoPermiso = (permitido === true || permitido === "SI" || permitido === "SÍ" || String(permitido).toUpperCase() === "SI") ? "SÍ" : "NO";

    if (filaIndex !== -1) {
      hoja.getRange(filaIndex, idxPerm + 1).setValue(estadoPermiso);
      ...
      const obj = {
        ...
        SUBMODULO: String(submodulo).toUpperCase(),
        ACCION: String(accion).toUpperCase(),
        PERMITIDO: estadoPermiso,
        ESTADO_PERMISO: "ACTIVO",
        FECHA_CREACION: ahora,
        FECHA_ACTUALIZACION: ahora,
        USUARIO_CREACION: usuario,
        USUARIO_ACTUALIZACION: usuario,
        OBSERVACIONES: "Permiso modificado dinámicamente desde Seguridad Web App"
      };
      hoja.appendRow(SEG_CONVERTIR_OBJETO_FILA(enc, obj));
    }
    ...
    SEG_REGISTRAR_AUDITORIA({
      ...
      ID_REGISTRO: idRol + "_" + modulo,
      DESCRIPCION: "Permiso " + modulo + " para " + idRol + " establecido a: " + estadoPermiso,
      RESULTADO: "EXITOSO"
    });

    return { EXITO: true, MENSAJE: "¡Matriz de permisos para '" + modulo + "' actualizada correctamente a " + estadoPermiso + "!" };
  } catch (error) {
